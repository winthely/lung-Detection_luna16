...
import copy
# K.set_image_dim_ordering('th')  # Theano dimension ordering in this code , tensorflow as tf
...
def gaussian_init(shape, name=None, dim_ordering=None):
   return initializers.normal(shape, scale=0.001, name=name, dim_ordering=dim_ordering)

def get_unet_small(options):
    inputss = Input((1, 512, 512))


    conv1 = Convolution2D(32, (3, 3), activation='elu', padding='same')(inputss)
    ...
    conv2 = Convolution2D(64, (3,3), activation='elu', padding='same')(pool1)
    ...
    conv3 = Convolution2D(128, (3,3), activation='elu', padding='same')(pool2)
    ...
    conv4 = Convolution2D(256, (3,3), activation='elu', padding='same')(pool3)
    ...
    # concatenate
    # up7 = concatenate([UpSampling2D(size=(2, 2))(conv4), conv3], mode='concat', concat_axis=1)
    up7 = Convolution2D(128, 2, activation = 'elu', padding = 'same')(UpSampling2D(size = (2,2))(conv4))
    merge7 = concatenate([conv3, up7], axis=1)
    conv7 = Convolution2D(128, 3, activation='elu', padding='same', kernel_initializer = 'he_normal')(merge7)
    conv7 = Dropout(0.2)(conv7)
    conv7 = Convolution2D(128, 3, activation='elu', padding='same',
                          kernel_initializer='he_normal')(conv7)
    conv7 = BatchNormalization()(conv7)

    # up8 = concatenate([UpSampling2D(size=(2, 2))(conv7), conv2], mode='concat', concat_axis=1)
    up8 = Convolution2D(64, 2, activation='elu', padding='same', kernel_initializer='he_normal')(
        UpSampling2D(size=(2, 2))(conv7))
    merge8 = concatenate([conv2, up8], axis=1)
    conv8 = Convolution2D(64, 3, activation='elu', padding='same', kernel_initializer='he_normal')(merge8)
    conv8 = Dropout(0.2)(conv8)
    conv8 = Convolution2D(64, 3, activation='elu', padding='same',
                          kernel_initializer='he_normal')(conv8)
    conv8 = BatchNormalization()(conv8)

    # up9 = concatenate([UpSampling2D(size=(2, 2))(conv8), conv1], mode='concat', concat_axis=1)
    up9 = Convolution2D(32, 2, activation='relu', padding='same', kernel_initializer='he_normal')(
        UpSampling2D(size=(2, 2))(conv8))
    merge9 = concatenate([conv1, up9], axis=1)
    conv9 = Convolution2D(32, 3, activation='elu', padding='same')(merge9)
    conv9 = Dropout(0.2)(conv9)
    conv9 = Convolution2D(32, 3, activation='elu', padding='same',
                          name='conv_9')(conv9)
    ...
    model = Model(inputs=inputss, outputs=conv10)
...
def train(use_existing):
    ...
    print('Fitting model...')
    print('-'*30)
    print(weight_save,accuracy)
    model.compile(optimizer=Adam(lr=options.lr, clipvalue=1., clipnorm=1.), loss=dice_coef_loss, metrics=[dice_coef])
    model.summary()
    model.fit(x=imgs_train, y=imgs_mask_train, batch_size=options.batch_size, epochs=options.epochs, verbose=1, shuffle=True
            ,callbacks=[accuracy])
              # callbacks = [accuracy])
